...
1. **Determine PR number**: Use the first text after the command as the PR number. If none provided, use the current PR
   number running `gh pr view --json number --jq '.number'`.
2. **Fetch diff**: Run `gh pr diff <number>` to get the PR changes (omit `<number>` when using current PR).
...
6. When commenting, add a comment specific to the file and line numbers of the changes.
   Also, add a comment with the overall summary of the changes.
